...
function getKeyframes(sheet, tracks, objectKey) {
  ...
  const { trackData, trackIdByPropPath } =
    json.sheetsById[sheet.address.sheetId].sequence.tracksByObject[objectKey];
  const keyframes = tracks.reduce((result, track) => {
    if (Array.isArray(track)) {
      const [prop1, prop2] = track;
      result[`${prop1},${prop2}`] = trackData[
        trackIdByPropPath[`["${prop1}","${prop2}"]`]
      ]?.keyframes?.map((k) => {
        return { position: k.position, value: k.value };
      });
    } else {
      result[track] = trackData[
        trackIdByPropPath[`["${track}"]`]
      ]?.keyframes?.map((k) => {
        return { position: k.position, value: k.value };
      });
    }
...
const deletrTracks = (tracks) => {
  ...
  tracks.forEach((track) => {
    getObjectbyId(studio.selection[0].address.objectKey).value[track] &&
      Object.keys(
        getObjectbyId(studio.selection[0].address.objectKey).value[track]
      )?.forEach((val) => {
  ...
  const keyframes = getKeyframes(
    sheet,
    tracksModified,
    studio.selection[0].address.objectKey
  );
  studio.transaction((api) => {
    tracksModified.forEach((track) => {
      if (Array.isArray(track)) {
        ...
        keyframes[track]?.forEach((val) => {
          sheet.sequence.position = val.position;
          api.unset(
            getObjectbyId(studio.selection[0].address.objectKey).props[track]
          );
        });
      }
    });
...
const DrawingforTheatrejs = ({
  ...
  const showExtensionPanel = useSelector(
    (state) => state.showExtensionPanelReducer.showExtensionPanel
  );
  const showSavePanel = useSelector(
    (state) => state.showSavePanelReducer.showSavePanel
  );
  const showDataUpdatePanel = useSelector(
    (state) => state.showDataUpdatePanelReducer.showDataUpdatePanel
  );
  const showHtmlOutput = useSelector(
    (state) => state.showHtmlOutputReducer.showHtmlOutput
  );
  const showDataTable = useSelector(
        ...
        set([
          {
            ...
            onClick: () => {
              dispatch({
                type: "SHOW_EXTENSIONPANNEL",
                payload: !showExtensionPanel,
              });
            },
          },
          {
          ...
          },
          {
            ...
            onClick: () => {
              dispatch({
                type: "SHOW_DATA_UPDATE",
                payload: !showDataUpdatePanel,
              });
            },
          },
          {
    ...
    <div id="aaa">
      ...
      >
        <div
          style={{
            ...
            display: showDataUpdatePanel ? "" : "none",
          }}
        >
          <div
            className="my-drag-handle2"
            style={{ width: 450, height: 20, backgroundColor: "white" }}
          >
            Drag me
            <button
      ...
      >
        <div
          style={{
            backgroundColor: "grey",
            display: showHtmlOutput ? "" : "none",
          }}
        >
          <div
            className="my-drag-handle"
            style={{ width: 1056, height: 20, backgroundColor: "white" }}
          >
            Drag me
            <button
              style={{ position: "absolute", right: 0 }}
      ...
      >
        <div
          style={{
            ...
            display: showDataTable ? "" : "none",
          }}
        >
          <div
            className="my-drag-handle"
            style={{ width: 1700, height: 20, backgroundColor: "white" }}
          >
            Drag me
            <button
              style={{ position: "absolute", right: 0 }}
      ...
      >
        ...
        >
          ...
          <TheatreImageSequence
            sheet={sheet}
            generateTheatreID={generateTheatreID}
            fps={FPS}
          />
        </div>
      </Rnd>
...
const loopDirection = [
  { direction: "normal", notation: "N" },
  { direction: "reverse", notation: "R" },
  { direction: "alternate", notation: "A" },
  { direction: "alternateReverse", notation: "AR" },
];
...
const WebAnimator = () => {
  ...
  useEffect(() => {
    if (canvas) {
      const handleKeyDown = (options) => {
        if (
          options.key === "Delete" &&
          document.activeElement === window.editor.canvas.wrapperEl
        ) {
          deleteItem(canvas);
        }
      };
  ...
  useEffect(() => {
    const handleSelectionChange = (newSelection) => {
      if (
        ...
        canvas &&
        newSelection[0]?.type === "Theatre_SheetObject_PublicAPI"
      ) {
        const selectedObject = canvas
          .getObjects()
          .find((item) => newSelection[0]?.address?.objectKey === item.id);
        if (selectedObject) {
        ...
        <ul>
          <li>
            ...
            <ul>
              ...
              <li onClick={() => addItem(createRect)}>
                Rectangle <VscPrimitiveSquare />
              </li>
              <li onClick={() => addItem(createRandomeStrip)}>
                Randome Path Strip <VscPrimitiveSquare />
              </li>
              <li onClick={() => addItem(createTextBox)}>Text T</li>
              <li onClick={() => addItem(createCircle)}>
                Circle <VscCircleFilled />
              </li>
              <li onClick={() => addItem(createTriangle)}>
                Triangle <VscTriangleUp />
              </li>
              <li onClick={appendDatafromLocalStorage}>
                Append Data from LocalStorage <VscTriangleUp />
              </li>
            </ul>
          </li>
          <li>
            ...
            <ul>
              ...
              {getObjectbyId(studio?.selection?.[0]?.address?.objectKey)
                ?.value &&
                Object.keys(
                  getObjectbyId(studio?.selection?.[0]?.address?.objectKey)
                    ?.value
          ...
          <li
            onClick={() => changeId(studio?.selection?.[0]?.address?.objectKey)}
          >
            Change Id
          </li>
          <li onClick={setSequenceLength}>Set sequence length</li>
          <li onClick={clearAllAnimation}>Clear All Animations</li>
          <li onClick={clearObjectsAllAnimation}>
            Clear Objects All Animations
          </li>
              ...
              <li
                onClick={() => {
                  const tracks = Object.keys(
                    getObjectbyId(studio.selection[0].address.objectKey).value
                  );
                  deletrTracks(tracks);
                }}
              >
          ...
          <li onClick={() => visibleInVisible1(canvas)}>
  ...
  useEffect(() => {
    onChange(sheet.sequence.pointer.position, (position) => {
      if (canvas) {
        const aa5 = canvas
          .getObjects()
          .find((element) => element.id === "imgSeqGroup1");
        aa5 &&
  ...
  const pause = (layerNumber) => {
    endpoint(
      `call ${window.chNumber}-${layerNumber} window.sheet.sequence.pause()`
    );
  ...
  const exportHtml = async (overRide = false) => {
    const mainPageData = JSON.stringify({
      duration,
      enableLoopAnimation,
      loopAnimationStart,
      loopAnimationEnd,
  ...
  const importHtml = async (canvasContent1, animationContetent1) => {
    ...
    if (canvasContent1) {
      ...
      if (animationContetent1 === undefined) {
        animationContetent1 =
          '{"sheetsById":{},"definitionVersion":"0.4.0","revisionHistory":["gjjL6UEXDCUdpAe_","nMdlSYh15PYUGb14"]}';
      }
      ...
      project.ready.then(() => {
        sheet.sequence.play({ range: [0, parseFloat(duration)] }).then(() =>
          sheet.sequence.play({
            iterationCount: enableLoopAnimation ? Infinity : 1,
            range: [
              parseFloat(loopAnimationStart),
              parseFloat(loopAnimationEnd),
            ],
  ...
  const processContent = (content) => {
    var canvasContent =
      content.split("const content =")[1].split("]};")[0] + "]}";
    var animationContetent = content.split("{state:")[1].split("});")[0];
    if (
      content.split("<script src='").length > 1 &&
      content.split("<script src='")[1].split(".").length > 1
    ) {
    ...
    try {
      const {
        ...
        content.split("const mainPageData=")[1].split("</script>")[0]
      );
      ...
      fpsfromfile1 = fpsfromfile;

      dispatch({ type: "CHANGE_FPS", payload: fpsfromfile });
      dispatch({
        type: "CHANGE_CURRENTSCREENSIZE",
    ...
    initialiseCore(canvasContent, true, fpsfromfile1);
  };
  const findElementWithId = (group, id) => {
    const objects = group.getObjects();
    for (let i = 0; i < objects.length; i++) {
  ...
  const generateTheatreIDforCopiedElement = (id, multiSelectedIndex = 0) => {
    ...
    indexOfSelectedElement = canvas
      .getObjects()
      .findIndex((obj) => obj.id === elementBeingCopied.id);
    ...
    arrObject[i] = sheet.object(element.id, {
      ...
      strkDsOfst: types.number(parseInt(objectBeingCopied.strkDsOfst), {
        range: [-1000, 1000],
      }),

      stroke: types.rgba({
        r: objectBeingCopied.stroke.r,
        g: objectBeingCopied.stroke.g,
        b: objectBeingCopied.stroke.b,
        a: objectBeingCopied.stroke.a,
      }),
      strokeWidth: types.number(objectBeingCopied.strokeWidth, {
        range: [0, 100],
      }),
      skewX: types.number(parseInt(objectBeingCopied.skewX), {
        range: [-88, 88],
      }),
      skewY: types.number(parseInt(objectBeingCopied.skewY), {
        range: [-88, 88],
      }),
      shadow: {
        ...shadowOptions,
        color: types.rgba({
          r: objectBeingCopied.shadow.color.r,
          g: objectBeingCopied.shadow.color.g,
          b: objectBeingCopied.shadow.color.b,
          a: objectBeingCopied.shadow.color.a,
        }),
        blur: types.number(parseInt(objectBeingCopied.shadow.blur), {
          range: [0, 100],
        }),
        offsetX: types.number(parseInt(objectBeingCopied.shadow.offsetX)),
        offsetY: types.number(parseInt(objectBeingCopied.shadow.offsetY)),
        affectStroke: objectBeingCopied.shadow.affectStroke,
      },
    });
    setOnValueChange(element, i);
  };
  ...
  const handleProcess = async (blob1) => {
    ...
    const processedBlob1 = new Blob([processedData.buffer], {
      type: "video/mov",
    });
      ...
      >
        ...
        >
          ...
          <button
            ...
            onClick={() => saveToLocalStorage(canvas)}
          >
            SaveTo Lo.Strg
          </button>
          <b>Ch:</b>
          ...
          <input
            ...
            value={idofElement}
            onChange={(e) =>
              setIdofElement(e.target.value.replace(/\s*\/\s*/g, " / "))
            }
          />
          ...
          <button onClick={() => pause(templateLayers.theatrejs)}>
            <FaPause />
          </button>
          <button
            title="Resume"
            onClick={() => resume(templateLayers.theatrejs)}
          >
            <FaPause />
            <FaPlay />
          </button>
          ...
          <button onClick={() => stopGraphics1(templateLayers.theatrejs)}>
            <FaStop />
          </button>
          <span title="Duration in Second">D:</span>
          <input
          ...
          <input
            type="checkbox"
            checked={enableLoopAnimation}
            onChange={() => setEnableLoopAnimation((val) => !val)}
          />
          <span>Loop Anim</span>
          <span>Start:</span>
          <input
            ...
            type="number"
            value={loopAnimationStart}
            style={{ width: 40 }}
            onChange={(e) => {
              if (e.target.value < loopAnimationEnd)
                setLoopAnimationStart(e.target.value);
            }}
          />
          <span>End:</span>
          <input
            title="Time in second"
            type="number"
            value={loopAnimationEnd}
            style={{ width: 40 }}
            onChange={(e) => {
              if (e.target.value > loopAnimationStart)
                setLoopAnimationEnd(e.target.value);
            }}
          />
          ...
          <span style={{ marginLeft: 5 }}>js:</span>
          <input
            type="text"
            style={{ width: 60 }}
            value={jsfilename}
          ...
          <span title="Html Client Id for html outPut">Client Id</span>
          <input
            title="For Html Rendrer. Put Unique Id so that other may not interfere"
            style={{ width: 50 }}
            type={"text"}
            value={clientId}
            onChange={(e) => {
          ...
          />
          <button disabled={recording ? true : false} onClick={() => record()}>
            {recording
              ? transcoding
                ? "Transcoding"
                : "Recoreding"
        ...
        <span
          style={{ position: "absolute", left: 960, top: 540, fontSize: 40 }}
        >
          .
        </span>
        <DrawingforTheatrejs
