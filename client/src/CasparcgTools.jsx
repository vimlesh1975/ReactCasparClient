    ...
    return (<div>
        ...
        <div style={{ marginTop: 20, border: '1px solid black', padding: 10 }}>
            ...
            <label>
                ...
                >
                    {inputs.map((val) => { return <option key={uuidv4()} value={val}>{val}</option> })}
                </select>
            </label>
            <button onClick={()=>{
                endpoint(`play ${window.chNumber}-1 DECKLINK ${selectedInput}`);
            }}>Show Input</button>
        </div>
    </div>)
}
