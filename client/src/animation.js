export const animation = [
    ...
    { name: 'keyframes rotate-in-2-cw', value: `@keyframes rotate-in-2-cw{0%{transform:rotate(-45deg);opacity:0}100%{transform:rotate(0);opacity:1}} div{animation:rotate-in-2-cw .5s cubic-bezier(.25,.46,.45,.94) both}` },
    ...
    {name:'animation:shake-horizontal', value:`@keyframes shake-horizontal{0%,100%{transform:translateX(0)}10%,30%,50%,70%{transform:translateX(-10px)}20%,40%,60%{transform:translateX(10px)}80%{transform:translateX(8px)}90%{transform:translateX(-8px)}} div {animation:shake-horizontal .8s cubic-bezier(.455,.03,.515,.955) both}`},
